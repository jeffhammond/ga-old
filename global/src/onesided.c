...
/* These are data hazards but not required. */
...
\
    /* initialize list */\
    for(int _i=0; _i< (nproc); _i++) ProcListPerm[_i]=_i;\
\
    /* list permutation generated by random swapping */\
    for(int _i=0; _i< (nproc); _i++){ \
      int iswap = (int)(rand() % (nproc));  \
      int temp = ProcListPerm[iswap]; \
...
{
#if 1
  pnga_error("this function is not thread-safe",nelems);
#else
...
{
#if 1
  pnga_error("this function is not thread-safe",0);
#else
  if (!GA_prealloc_gatscat)
                     ...
                     double* totbytes, void *alpha)
{
    ...
    /* Avoid thread-unsafe GA_header, GA_list, GA_elems completely */
    /* if (!GA_prealloc_gatscat) { */
